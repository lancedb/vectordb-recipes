...
async function processImgs(imgs_dir: string, db: any){
  ...
  for (var i = 0; i < imgs.length; i++) {
    img_files.push((await sharp(path.join(process.cwd(), imgs_dir , imgs[i])).resize(200,200).jpeg({quality : 50}).toBuffer()).toString('base64'));
  }
...
export async function createEmbeddingsTable() {
  ...
  if (tableNames.includes(LANCEDB_TABLE_NAME))
    return LANCEDB_TABLE_NAME;

  // Exec output contains both stderr and stdout outputs
  console.log("Downloading COCO128 dataset")
  await exec('wget https://ultralytics.com/assets/coco128.zip -O public/coco128.zip')
