...
def build_vector_store(text):
    ...
    st.session_state["vector_store"] = LanceDB.from_texts(
        text_chunks, st.session_state["embed_model"]
    )
...
def fetch_context(query):
    return st.session_state["vector_store"].similarity_search(
        query, k=st.session_state["top_k"]
    )
...
def user_input(user_question):
    ...
    st.session_state["eval_models"]["app_metrics"].exec_times[
        "chunk_fetch_time"
    ] = exec_time
...
def evaluate_all(query, context_lis, response):
    ...
    RESULT["execution_times"] = st.session_state["eval_models"][
        "app_metrics"
    ].exec_times
...
def main():
    ...
    st.error(
        "WARNING: If you reload the page, everything (model, PDF, key) will have to be loaded again. That's how `streamlit` works",
        icon="🚨",
    )
    ...
    with st.sidebar:
        ...
        st.session_state["pdf"] = st.file_uploader(
    ...
    if not st.session_state["api_key"]:
        ...
        if (
            user_question and st.session_state["api_key"]
            ...
            st.warning(
                "There are 5 major types metrics computed below having multiple sub metrics. Also, 2 abstract classes are defined `LLMasEvaluator` (to use any LLM as a judge) and `TraditionalPipelines` (for Topics, NER, POS etc)",
                icon="🤖",
            )
            ...
            if metric_calc:
                ...
                with st.spinner("Calculating all the matrices. Please wait ...."):
                    eval_result = evaluate_all(
                        user_question,
                        [item.page_content for item in contexts_with_scores],
                        response,
                    )
                    st.balloons()
