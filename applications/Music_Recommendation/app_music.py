...
st.sidebar.markdown(
    "Choose a sample track to test or upload your own audio file (MP3 format only) to get music recommendations based on the song."
)
...
if query_mp3_path:
    ...
    (_, query_embedding) = at.inference(
        np.expand_dims(query_audio_array, axis=0)
    )  # Add batch dimension
...
st.markdown("---")
st.markdown(
    "**How It Works:** Upload a song or choose a sample, and we'll use Lancedb vectordb to find other tracks with similar audio features. Enjoy your personalized music recommendations! 🎧"
)
