...
def insert(video_ids):
    ...
    if "videos" in db.table_names():
        ...
        table.add([{'vector': im, 'text': title, 'video_id': vid, 'start_time': 0} for (im, vid, title) in zip(text_features, video_ids, titles)])
    else:
        db.create_table("videos", [{'vector': im, 'text': title, 'video_id': vid, 'start_time': 0} for (im, vid, title) in zip(text_features, video_ids, titles)])
    print("done")
